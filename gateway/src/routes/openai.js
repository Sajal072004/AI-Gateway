...
export async function registerOpenAIRoutes(fastify) {
    ...
    /**
     * POST /v1/completions
     * OpenAI-compatible completions endpoint for autocomplete/inline completion
     */
    fastify.post('/v1/completions', {
        ...
        handler: async (request, reply) => {
            ...
            const requestId = generateRequestId();
            const userId = request.userId;
            const { prompt, model: requestedModel, max_tokens, temperature, stream } = request.body;
            ...
            try {
                ...
                if (requestedModel) {
                    ...
                    else if (requestedModel === 'auto') requestedTierFromBody = 'auto';
                }

                // Default to cheap for completions (autocomplete is typically fast/cheap)
                if (!requestedTierFromBody && !requestedModel) {
                    requestedTierFromBody = 'cheap';
                }

                let requestedTier = determineRequestedTier({ tier: requestedTierFromBody }, userPolicy);
                let routingReason = 'explicit';

                // Convert prompt to messages format for internal processing
                ...
                if (!limitCheck.allowed) {
                    // Try fallback to cheap if allowed
                    let fallbackSuccess = false;
                ...
                // Format response as OpenAI Completions (Non-streaming)
                return reply.send({
                    id: `cmpl-${requestId}`,
                    object: 'text_completion',
                    created: Math.floor(Date.now() / 1000),
