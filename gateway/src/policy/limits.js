...
export async function checkLimits(userId, tier, estimatedPromptTokens, day, month, userPolicy, systemPolicy) {
    ...
    // Get limits
    const userDailyTokenLimit = userPolicy.dailyTokenLimit[tier];
    const userMonthlyTokenLimit = userPolicy.monthlyTokenLimit[tier];
    const userDailyRequestLimit = userPolicy.dailyRequestLimit[tier];
    const userMonthlyRequestLimit = userPolicy.monthlyRequestLimit[tier];

    const globalDailyTokenLimit = systemPolicy.globalDailyTokenLimit[tier];
    const globalMonthlyTokenLimit = systemPolicy.globalMonthlyTokenLimit[tier];
    const globalDailyRequestLimit = systemPolicy.globalDailyRequestLimit[tier];
    const globalMonthlyRequestLimit = systemPolicy.globalMonthlyRequestLimit[tier];
...
export async function reserveRequest(userId, tier, day, month) {
    ...
    await UsageRollup.findOneAndUpdate(
        { periodType: 'day', period: day, scope: 'user', userId, tier },
        { $inc: { requests: 1 } },
    ...
    // Update user month
    await UsageRollup.findOneAndUpdate(
        { periodType: 'month', period: month, scope: 'user', userId, tier },
        { $inc: { requests: 1 } },
        { upsert: true, new: true }
    );
    ...
    // Update global day
    await UsageRollup.findOneAndUpdate(
        { periodType: 'day', period: day, scope: 'global', userId: null, tier },
        { $inc: { requests: 1 } },
        { upsert: true, new: true }
    );
    ...
    // Update global month
    await UsageRollup.findOneAndUpdate(
        { periodType: 'month', period: month, scope: 'global', userId: null, tier },
        { $inc: { requests: 1 } },
        { upsert: true, new: true }
    );
}
